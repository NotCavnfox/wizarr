...
WORKFLOW = (
    Path(__file__).resolve().parents[1] / ".github/workflows/fork-sync-upstream.yml"
)
...
""")
        ...
        self.env = dict(
            ...
            GITHUB_REPOSITORY="NotCavnfox/" + UPSTREAM_REPO.rsplit("/", 1)[1],
            GITHUB_RUN_ID="123",
    ...
    def gh_calls(self):
        return (
            [json.loads(s) for s in self.calls.read_text().splitlines()]
    ...
    def test_calendar_stable_release_is_accepted(self):
        ...
        self.assertEqual(
            self.env["UPSTREAM_SHA"], self.git(self.upstream, "rev-parse", "v2026.9.0")
        )
    ...
    def test_clean_sync_retry_preserves_head_and_successful_ci(self):
        ...
        self.run_step(4)
        self.assertEqual(
            self.git(self.origin, "rev-parse", "refs/heads/" + branch), head
        )
        calls = self.gh_calls()
        ...
        pushes = [
            json.loads(c)
            for c in self.git_calls.read_text().splitlines()
            if json.loads(c)[0] == "push"
        ]
        self.assertEqual(len(pushes), 1)
        self.assertFalse(any(a.startswith(("-f", "+")) for c in pushes for a in c[1:]))
    ...
    def test_divergent_existing_branch_is_preserved(self):
        ...
        self.assertEqual(
            self.git(self.origin, "rev-parse", "refs/heads/" + branch), divergent
        )
    ...
    def test_same_tree_without_fork_history_is_rejected(self):
        ...
        rewritten = self.git(
            ...
            tree,
            "-p",
            self.env["UPSTREAM_SHA"],
            "-m",
            "same tree without fork history",
        )
        self.git(self.work, "push", "origin", rewritten + ":refs/heads/" + branch)
        self.run_step(4, expected=1)
        self.assertEqual(
            self.git(self.origin, "rev-parse", "refs/heads/" + branch), rewritten
        )
        self.assertFalse(
            any(
                c[:2] in [["pr", "create"], ["workflow", "run"]]
                for c in self.gh_calls()
            )
        )
